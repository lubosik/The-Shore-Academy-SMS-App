...
async function sendNativeMessagePush({ title, body, phone }) {
  ...
  // Fall back on EMPTY as well as on error. The ios_push_devices migration was
  // never run on this project, so registration lands in compatibility storage
  // and this table exists but stays empty — which returns zero rows and no
  // error. Falling back only on error therefore found no devices and silently
  // sent nothing, which is how push looked "fine" while delivering nothing.
