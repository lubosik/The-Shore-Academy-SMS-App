...
async function upsertGhlContact(c, fallbackSource = 'ghl') {
  ...
  // GHL's own source ("Miami Beach Landing Page", "Deerfield Ad Lead") says
  // which ad or page produced the lead, which is worth more than our generic
  // import label. Set on update as well as insert — but only when GHL actually
  // has one, per the rule above about never blanking existing data.
